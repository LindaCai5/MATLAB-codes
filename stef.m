function root = stef(f,x,xtol,ftol,n_max,display,myroot)
...
rel = abs(x-myroot)/abs(myroot);
if display, 
   disp('   n             x                    f(x)                     rel')
   disp('----------------------------------------------------------------------------')
   disp(sprintf('%4d %23.15e %23.15e %23.15e', n, x, fx, rel))
end
if abs(fx) <= ftol
   root = x;
   return
...
for n = 1:n_max
    v = feval(f,x);
    gx = (feval(f,(x+v))-v)/v; % fd is either a string name or the handle of f(x)
...
%    fx = f(x);   % f is the handle of f(x)  
    rel = abs(x-myroot)/abs(myroot);
    if display, 
       disp(sprintf('%4d %23.15e %23.15e %23.15e', n, x, fx, rel)), end
